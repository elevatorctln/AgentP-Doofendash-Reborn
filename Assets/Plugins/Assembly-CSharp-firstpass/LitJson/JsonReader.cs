...
namespace LitJson
{
	public class JsonReader
	{
		...
		private void ProcessNumber(string number)
		{
			...
			int n_int;
			long n_int2;
			if ((number.IndexOf('.') != -1 || number.IndexOf('e') != -1 || number.IndexOf('E') != -1) && TryParseDouble(number, out n_double))
			{
				token = JsonToken.Double;
				token_value = n_double;
			}
			else if (TryParseInt32(number, out n_int))
			{
				token = JsonToken.Int;
				token_value = n_int;
			}
			else if (TryParseInt64(number, out n_int2))
			{
